...
Let's also save the predicted values in a separate vector.
...
Let's save the predictions for later on.
...
Let's save the prediction values.
...
Let's calculate an averaged solution, from the 3 solutions we saved.

```{r averaged_solution}
testset$predicted = 0.3 * predicted_rf + 0.3 * predicted_gbm + 0.4 * predicted_xgb
```

Let's see the ROC-AUC score for the weighted averaged solution.
...
At the end, we also provided an weighted averaged solution (using RF, GBM and XGB solutions).
